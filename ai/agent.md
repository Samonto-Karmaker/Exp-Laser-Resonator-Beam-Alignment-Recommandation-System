...
- **Micro-Steps**: When tasked with a specific project phase, strictly read the relevant `ai/implementation/step_*_explainer.md` file *before* writing code.
- **State Management**: Regularly view and update `ai/implementation/PROGRESS.md` to mark step boundaries.
...
- Follow the evaluation protocol strictly to benchmark against both random *and* grouped (by experiment) splits.

### 4. Completion & Documentation
- Ensure every major change successfully propagates up through documentation.
- After completing a step, check off its status in `ai/implementation/PROGRESS.md`.
